...
class Rental
  ...
  def calculate_driver_amount
    self.full_price + self.calculate_deductible_reduction_amount
  end
  ...
  def compute_actions
    ...
    ACTORS.each do |stakeholder|
      actions << {
        ...
        "amount" => self.send("calculate_#{stakeholder[:who]}_amount".to_sym).round
        }
    end
    actions
  end
